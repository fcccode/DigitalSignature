...
expectedRes = 0
	...
	ActiveCertPath = "test45.p12"
	crlList5(0) = "Trust Anchor CRL CP.01.01.crl"
	crlList5(1) = "Intermediate CRL 1 PP.06.03.crl"
	crlList5(2) = "Intermediate CRL 2 PP.06.03.crl"
	crlList5(3) = "Intermediate CRL 3 PP.06.03.crl"
	crlList5(4) = "Intermediate CRL 4 PP.06.03.crl"
	...
	if res = expectedRes then
		fHandle.writeline("Test 45 Successful. ipolset=any policy, explicit=false. Certificate Validation Failed " & "Last Error: " & SecureXML.GetLastError)
	else
...
expectedRes = 0
	...
	res = SecureXML.VerifyPFXCertCRL(ActiveCertPath, "password", "", 1)
	if res = expectedRes then
		fHandle.writeline("Test 45 Successful. ipolset=any policy, explicit=true. Certificate Validation Failed " & "Last Error: " & SecureXML.GetLastError)
	else
		fHandle.writeline("Test 45 Failed. Certificate Validated Successfully")
	End if
