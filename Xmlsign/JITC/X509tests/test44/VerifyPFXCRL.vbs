...
expectedRes = 1
	...
	ActiveCertPath = "test44.p12"
	crlList5(0) = "Trust Anchor CRL CP.01.01.crl"
	crlList5(1) = "Intermediate CRL 1 PP.06.02.crl"
	crlList5(2) = "Intermediate CRL 2 PP.06.02.crl"
	crlList5(3) = "Intermediate CRL 3 PP.06.02.crl"
	crlList5(4) = "Intermediate CRL 4 PP.06.02.crl"
	...
	if res = expectedRes then
		...
		fHandle.writeline("Test 44 Failed. Certificate Validation Failed " & "Last Error: " & SecureXML.GetLastError)
	End if
...
expectedRes = 0
	...
	res = SecureXML.VerifyPFXCertCRL(ActiveCertPath, "password", "", 1)
	if res = expectedRes then
		fHandle.writeline("Test 44 Successful. ipolset=any policy, explicit=true. Certificate Validation Failed " & "Last Error: " & SecureXML.GetLastError)
	else
