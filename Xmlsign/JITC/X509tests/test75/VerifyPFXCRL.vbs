...
expectedRes = 0
	...
	ActiveCertPath = "test75.p12"
	crlList2(0) = "Trust Anchor CRL CP.01.01.crl"
	crlList2(1) = "Intermediate CRL RL.08.01.crl"
	SecureXML.CRLLocation = crlList2

	res = SecureXML.VerifyPFXCertCRL(ActiveCertPath, "password", "", 1)
	if res = expectedRes then
		MsgBox("Test 75 Successful. Certificate Validation Failed " & "Last Error: " & SecureXML.GetLastError)
	else
		MsgBox("Test 75 Failed. Certificate Validated Successfully")
	End if
