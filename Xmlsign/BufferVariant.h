...
public:
	...
	VARIANT Copy();
	void MyVariantCopy(VARIANTARG *, VARIANTARG *);
...
{
	long nLength, uBound, lBound;
	SafeArrayGetUBound(varSrc.m_varData.parray, 1, &uBound);
	SafeArrayGetLBound(varSrc.m_varData.parray, 1, &lBound);
...
//	pSafeArrayBound[0].cElements = nLength+1; //Why??
	pSafeArrayBound[0].cElements = nLength;
...
//	::VariantCopy(&vNew, &m_varData);
	MyVariantCopy(&vNew, &m_varData);
	return vNew;
}

inline void CBufferVariant::MyVariantCopy(VARIANTARG *vDest, VARIANTARG *vSrc)
{
	if (vDest == vSrc)
		return;
	VariantClear(vDest);
	...
	*vDest = *vSrc;
	...
	SafeArrayCopy(vSrc->parray, &vDest->parray);
}
...
inline long CBufferVariant::GetLength()
{
	...
	long nLength, lBound, uBound;
	SafeArrayGetUBound(m_varData.parray, 1, &uBound);
	SafeArrayGetLBound(m_varData.parray, 1, &lBound);
	nLength = uBound - lBound + 1; // For C#
//	if (builtFromVariant) nLength++;
	return nLength;
}
...
inline void CBufferVariant::BuildByteArrayFromVariant(const VARIANT* pSrc)
{
	...
	if ((V_VT(pSrc) == (VT_ARRAY | VT_UI1)) && SafeArrayGetDim(pSrc->parray) == 1)
	{
		...
		//+ Access the data  
		SafeArrayAccessData(m_varData.parray, (void HUGEP**)&m_pBuffer);
//		builtFromVariant = true;
	}
	else
		if (V_VT(pSrc) == (VT_VARIANT | VT_BYREF))
		{
			VARIANT *ppSrc = pSrc->pvarVal;
			if ((V_VT(ppSrc) == (VT_ARRAY | VT_UI1)) && SafeArrayGetDim(ppSrc->parray) == 1)
			{
				::VariantCopy(&m_varData, const_cast<VARIANT*>(ppSrc));
				// Lock and access the memory
				//+ Access the data  
				SafeArrayAccessData(m_varData.parray, (void HUGEP**)&m_pBuffer);
//				builtFromVariant = false;
			}
			else
			{
				SetLength(0);
