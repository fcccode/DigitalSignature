...
public class TinySSL extends Socket {
    ...
    public static void main(String[] args) {
        Log.on = true;
        try {
            Socket s = new TinySSL("linux71", 443, true, true);
                ...
                case 11: // Server's certificate(s)
                    ...
                    for(int i=0; i<numcertbytes;) {
                        ...
                        if (server_cert == null) {
                            ...
                            String s0 = tbs.getStartDate().getTime();
                            s0 = s0.substring(0, 4) + "-" + s0.substring(4, 6) + "-" + s0.substring(6, 8) + "-" +
                                s0.substring(8, 10) + "-" + s0.substring(10, 12) + "-" +
                                s0.substring(12, 14) + "-" + s0.substring(14);

                            Date startDate = dateF.parse(s0, new ParsePosition(0));

                            String s1 = tbs.getEndDate().getTime();
                            s1 = s1.substring(0, 4) + "-" + s1.substring(4, 6) + "-" + s1.substring(6, 8) + "-" +
                                s1.substring(8, 10) + "-" + s1.substring(10, 12) + "-" +
                                s1.substring(12, 14) + "-" + s1.substring(14);

                            Date endDate = dateF.parse(s1, new ParsePosition(0));
    ...
    static {
        ...
        try {
            ...
            for(int i=0; i<base64_encoded_trusted_CA_public_keys.length; i+=2) {
                ...
                org.bouncycastle.asn1.DERObject dero = (DERObject) o;
                trusted_CA_public_keys[i/2] = new SubjectPublicKeyInfo((DERSequence)dero);
            }
