...
SECU_PrintCertNickname(CERTCertificate *cert, void *data)
{
	...
	fprintf(out, "%-60s %-5s\n", name, trusts);
	SECU_PrintName(out, &cert->subject, "Subject", 1);
	// fprintf(out, "Subject : %s\n", cert->subject);
	fprintf(out, "SubjectName : %s\n", cert->subjectName);
	SECU_PrintInteger(out, &cert->serialNumber, "Serial Number", 1);
	// fprintf(out, "Serial Num : %s\n", cert->serialNumber);
	fprintf(out, "timeOK : %s\n", cert->timeOK);
	SECU_PrintName(out, &cert->issuer, "Issuer", 1);
	// cert->derCert
	// ******
	// cert->derCert Base64 encode this to get X509 data.
	// ******
