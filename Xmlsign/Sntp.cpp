...
CNtpTime::CNtpTime(const SYSTEMTIME& st)
{
  ...
  ATLASSERT(JD >= 0);//NTP only supports dates greater than 1900
...
void CNtpSocket::Close()
{
	...
	{
		ATLASSERT (SOCKET_ERROR != closesocket(m_hSocket));
...
BOOL CSNTPClient::GetServerTime(LPCTSTR pszHostName, NtpServerResponse& response, int nPort)
{
  ...
  ATLASSERT(pszHostName);
	...
	if (!pSocket->Create())
	{
		ATLTRACE(_T("Failed to create client socket, GetLastError returns: %d\n"), GetLastError());
		return FALSE;
	}
	...
	{
		ATLTRACE(_T("Could not connect to the SNTP server %s on port %d, GetLastError returns: %d\n"), pszHostName, nPort, GetLastError());
	...
	else
	{
		...
		{
		ATLTRACE(_T("Failed in call to send NTP request to the SNTP server, GetLastError returns %d\n"), GetLastError());
		...
		{
		ATLTRACE(_T("Unable to wait for NTP reply from the SNTP server, GetLastError returns %d\n"), WSAETIMEDOUT);
		...
		{
		ATLTRACE(_T("Unable to read reply from the SNTP server, GetLastError returns %d\n"), GetLastError());
...
BOOL CSNTPClient::EnableSetTimePriviledge()
{
  ...
  if (!bOpenToken) 
  {
    ...
    ATLTRACE(_T("Failed to get Adjust priviledge token\n"));
    return FALSE;
  }
  ...
  {
    ATLTRACE(_T("Failed in callup to lookup priviledge\n"));
    return FALSE;
  }
  ...
  BOOL bSuccess = AdjustTokenPrivileges(m_hToken, FALSE, &m_TokenPriv, 0, NULL, 0);
	if (!bSuccess)
	  ATLTRACE(_T("Failed to adjust SetTime priviledge\n"));
...
void CSNTPClient::RevertSetTimePriviledge()
{
  if (m_bTakenPriviledge) 
  {
    ...
    if (!AdjustTokenPrivileges(m_hToken, FALSE, &m_TokenPriv, 0, NULL, 0))
		  ATLTRACE(_T("Failed to reset SetTime priviledge\n"));
  }
}
...
BOOL CSNTPClient::SetClientTime(const CNtpTime& NewTime)
{
  ...
  if (EnableSetTimePriviledge())
  {
    ...
    if (!bSuccess)
      ATLTRACE(_T("Failed in call to set the system time\n"));
  }
